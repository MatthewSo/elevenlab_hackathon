...
from backend.llm.context_generators import get_evaluation_context, get_explanation_context
from backend.llm.interface.statement_evaluator import StatementEvaluator, StatementEvaluation
...
class MistralStatementEvaluator(StatementEvaluator):
    ...
    def generate_explanation(self, statement: str, evaluation: StatementEvaluation):
        content_str = get_explanation_context(self.background_info, statement, evaluation)
        chat_response = self.client.chat.complete(
            model= self.model,
            messages = [
                {
